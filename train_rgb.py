...
if __name__ == '__main__':
    ...
    train_set = transforms.Transform(
        ...
        transform=transforms.Sequence(
            (transforms.FromDataDictKeys(keys=('rgb',)),
             transforms.NumpyToTorch(keys=('rgb',)),
             transforms.TensorChannelFirst(keys=('rgb',)),

             transforms.GetSVD(hsi_key='rgb', svd_key='svd'),

             transforms.TensorRandomCrop(size=config.train_crop_size, keys=('rgb',)),
             transforms.TensorRandomRotate(keys=('rgb',)),
             transforms.TensorRandomFlip(p=config.train_random_flip_p, keys=('rgb',)),

             transforms.GetSeqDegHSI(
                 degradation_lst=(ImgDownscale(scale_factor=1 / config.scale),),
                 hsi_key='rgb', deg_hsi_key='deg_rgb'
             ),

             transforms.GetSVDEigenimages(
                 hsi_key='rgb', deg_hsi_key='deg_rgb', svd_key='svd',
                 eigen_key='eigen', deg_eigen_key='deg_eigen'
             ),
             transforms.GetOneChannel(idx=0, keys=('eigen', 'deg_eigen')),
